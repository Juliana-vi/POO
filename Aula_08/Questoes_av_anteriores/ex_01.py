...
class Triangulo():
    def __init__(self):   #Encapsulamento
        self.b = 0
        self.h = 0
    def set_b(self, f):  # método de acesso: set para ajustar o valor do atributo
        if f < 0: raise ValueError("A base não pode ser negativa")   # validação
        self.__b = f
    def set_h(self, f): 
        if f < 0: raise ValueError("A altura não pode ser negativa")  
        self.__h = f
